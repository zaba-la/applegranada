...
export function resetPasswordEmailHtml(params: { name: string; link: string }) {
  const { name, link } = params;
  return `
<!DOCTYPE html>
<html lang="es">
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.08);">
        ...
        <tr>
          <td style="padding:32px;">
            ...
            <p style="margin:0 0 24px;color:#666;font-size:15px;">
              Hola ${name}, recibimos una solicitud para restablecer la contraseña de tu cuenta.<br>
              Haz clic en el botón para crear una contraseña nueva. Si no lo solicitaste, ignora este mensaje.
            </p>
            <table cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
              <tr>
                <td style="background:#000;border-radius:8px;padding:14px 28px;">
